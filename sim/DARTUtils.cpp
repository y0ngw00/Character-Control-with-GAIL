...
Character*
DARTUtils::
buildFromFile(const std::string& path, Eigen::Vector4d color_filter, bool isContact, bool isBVH)
{
	TiXmlDocument doc;
	...
	std::vector<dart::dynamics::BodyNode*> end_effectors;
	Eigen::VectorXd kps, maxfs, w_joint, ms, ds, ss;
	std::vector<std::string> bvh_map;
	for(TiXmlElement* node = skeleton_elem->FirstChildElement("Node");node != nullptr;node = node->NextSiblingElement("Node"))
	{
		...
		if(joint->Attribute("endeffector")!=nullptr){
			std::string ees = joint->Attribute("endeffector");
			if (ees =="True")
				ee = true;
		}
		...
		std::string weightstring = joint->Attribute("weight");
		double weight = stringToVector1d(weightstring)[0];

		std::string mstring = joint->Attribute("m");
		double m = stringToVector1d(mstring)[0];

		std::string dstring = joint->Attribute("d");
		double d = stringToVector1d(dstring)[0];

		std::string kstring = joint->Attribute("k");
		double k = stringToVector1d(kstring)[0];

		std::string bvhstring = "None";
		if(joint->Attribute("bvh") != nullptr)
			bvhstring = joint->Attribute("bvh");
		bvh_map.emplace_back(bvhstring);
		...
		else if(type == "Ball" || isBVH)
		{
			...
			{
				lower = stringToVector3d(joint->Attribute("lower"));
				upper = stringToVector3d(joint->Attribute("upper"));
			}
		...
		if(ee)
			end_effectors.emplace_back(bn);
		if(dof>0)
		{
			Eigen::VectorXd old_kps = kps;

			kps.resize(old_kps.rows()+dof);
			kps.head(old_kps.rows()) = old_kps;
			kps.tail(dof) = Eigen::VectorXd::Constant(dof,kp);

			Eigen::VectorXd old_maxfs = maxfs;

			maxfs.resize(old_maxfs.rows()+dof);
			maxfs.head(old_maxfs.rows()) = old_maxfs;
			maxfs.tail(dof) = Eigen::VectorXd::Constant(dof,maxf);


		}
		Eigen::VectorXd old_w_joint = w_joint;

		w_joint.resize(old_w_joint.rows()+1);
		w_joint.head(old_w_joint.rows()) = old_w_joint;
		w_joint[old_w_joint.rows()] = weight;

		Eigen::VectorXd old_m = ms;

		ms.resize(old_m.rows()+1);
		ms.head(old_m.rows()) = old_m;
		ms[old_m.rows()] = m;

		Eigen::VectorXd old_d = ds;

		ds.resize(old_d.rows()+1);
		ds.head(old_d.rows()) = old_d;
		ds[old_d.rows()] = d;

		Eigen::VectorXd old_s = ss;

		ss.resize(old_s.rows()+1);
		ss.head(old_s.rows()) = old_s;
		ss[old_s.rows()] = k;
	}
